...
const addTimeslots = async (timeslots, projectId = null, volunteerId = null) => {
  ...
  await models.Timeslot.bulkCreate(slotsWithId).catch((err) => console.log(err));
};
...
const addTimeslot = async (req, res) => {
  ...
  const newSlot = await models.Timeslot.create(timeslotJson).catch((err) =>
...
const editTimeslotRest = async (req, res) => {
  ...
  const currTimeslot = await models.Timeslot.findByPk(id).catch((err) => (findError = err));
  ...
  await currTimeslot.update(timeslotJson).catch((err) => (updateError = err));
...
const deleteTimeslotRest = async (req, res) => {
  ...
  const timeslot = await models.Timeslot.findByPk(req.params.id).catch((err) => (findError = err));
  ...
  if (timeslot) {
    await timeslot.destroy().catch((err) => (deleteError = err));
