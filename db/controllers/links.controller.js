...
const addLinks = async (links, projectId) => {
  ...
  await Link.bulkCreate(linksWithId).catch((err) => console.log(err));
};
...
const addLinkRest = async (req, res) => {
  let error;
  const { title, url, required, projectId } = req.body;
...
const editLinkRest = async (req, res) => {
  ...
  const { title, url, required } = req.body;

  const link = await Link.findByPk(req.params.id).catch((err) => (findError = err));
  ...
  if (link) {
    const result = await link.update({ title, url, required }).catch((err) => (updateError = err));
...
const removeLinkRest = async (req, res) => {
  ...
  if (link) {
    await link.destroy().catch((err) => (deleteError = err));
