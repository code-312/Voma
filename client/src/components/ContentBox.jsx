...
} from '../styles/components/ContentBox.style';
...
const ContentBox = ({
  ...
  return (
    <Card marginTop={marginTop} hideScroll={hideScroll}>
      ...
      <ContentBoxLinkContainer variant={variant}>
        {links.map((link, index) => (
          <HeaderLinkContainer active={activePage === index} key={link}>
            <HeaderLink onClick={() => handleHeaderClick(index)} variant={variant}>
              {link}
            </HeaderLink>
          </HeaderLinkContainer>
        ))}
      ...
      <ContentBoxFootContainer variant={variant}>{footContent}</ContentBoxFootContainer>
    </Card>
  );
};
