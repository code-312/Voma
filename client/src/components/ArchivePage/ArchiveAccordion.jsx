...
const ArchiveAccordion = ({ project, triggerDeleteModal, triggerReactivateModal }) => {
  const headerContent = (
    <ArchiveHeader>
      ...
      <BodySubText>
        Date Archived: {new Date(project.updatedAt).toLocaleString('en-US')}
      </BodySubText>
    </ArchiveHeader>
  );
  ...
  return (
    <Accordion header={headerContent}>
      ...
      <ArchiveFooter>
        <Button variant="solid blue" onClick={() => triggerReactivateModal(project.id)}>
          Add to Projects
        </Button>
        <Button variant="solid red" onClick={() => triggerDeleteModal(project.id)}>
          Delete
        </Button>
      </ArchiveFooter>
    </Accordion>
