...
module.exports = {
   'Login Form (Successful Login)': function(browser) {
        ...
        browser
           ...
           .getCookie('connect.sid', function(result) {
               this.assert.equal(result.name, 'connect.sid', '(Session) Check session cookie {connect.sid} is set.');
           })
           .url(function(result) {
               this.assert.equal(result.value, 'http://localhost:3000/board', '(Login Form) Verify successful login redirected to /board');
           })
           .deleteCookie('connect.sid')
           .refresh()
           .pause(1000)
           .url(function(result) {
               this.assert.equal(result.value, 'http://localhost:3000/login', '(Session) Verify deleting session cookie {connect.sid} redirects user to /login on refresh');
           })
           .end();
   ...
   'Login Form (Failed Login)': function(browser) {
        ...
        browser
            ...
            .pause(1000)
            .url(function(result) {
                this.assert.equal(result.value, 'http://localhost:3000/login', '(Login Form) Verfiy failed login did not redirect to authenticated page.');
            })
            .url('http://localhost:3000/board')
            .pause(1000)
            .url(function(result) {
                this.assert.equal(result.value, 'http://localhost:3000/login', '(Session) Verify that after failed login attempt, authenticated pages redirect to /login.');
            })
            .end();
