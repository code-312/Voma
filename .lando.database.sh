...
# Add test Projects.
for i in "${projects[@]}" ; do echo "insert into projects (name, description, created_at, updated_at) values ('$i', '$i', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);" >> seed.psql; done

# Add test Skills 
for i in "${skills[@]}" ; do echo "insert into skills (name, description, created_at, updated_at) values ('$i', '$i', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);" >> seed.psql; done

# Add test Volunteers assigned to random projects.
for i in "${volunteers[@]}" ; do echo "insert into volunteers (name, email, slack_user_id, pronouns, project_id, created_at, updated_at) values ('$i', 'fake.volunteer.$(((RANDOM%99999)+1))@test.null', 'U03AU1WDP5Z', 'lorum/ipsum', '$(((RANDOM%4)+1))', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);" >> seed.psql; done

# Add test Volunteers assigned to no projects.
for i in "${unassigned_volunteers[@]}" ; do echo "insert into volunteers (name, email, slack_user_id, pronouns, created_at, updated_at) values ('$i', 'fake.volunteer.$(((RANDOM%99999)+1))@test.null', 'U03AU1WDP5Z', 'lorum/ipsum', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);" >> seed.psql; done

# Add test Volunteer Skills.
for ((n=1; n<=15; n++)) ; do echo "insert into \"VolunteerSkills\" (volunteer_id, skill_id, created_at, updated_at) values ('$n', '$(((RANDOM%6)+1))', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);" >> seed.psql; done
